...
void writeCallback(std::string *content, WritableTable *table, int &depth){
    depth += 1;

    std::string tempIndent;
    for (int i = 0; i < depth - 1; i++){
        tempIndent += "\t";
    }

	*content += tempIndent + "?" + table->m_tableName + " {\n";

	// Loop over the values, and write to the content
	for (const auto& k : table->m_values){
		std::string key = k.first;
		std::string val = k.second;

        std::string tempIndent2;
        for (int i = 0; i < depth; i++){
            tempIndent2 += "\t";
        }

        *content += tempIndent2 + "@" + key + " = " + val + "\n" ;
	}

	
	// Loop over the subtables and write call back to ourself
	for (WritableTable &subtable : table->m_subtables){
		writeCallback(content, &subtable, depth);
	}
...
void SubTableWriter::write(){
	...
	for (WritableTable &table : m_root->m_subtables){
		writeCallback(&content, &table, depth);
	}
